...
export const apiKeyConfig = Config.redacted("API_KEY_INDEXINGCO").pipe(
  Config.orElse(() => Config.redacted("API_KEY_INDEXING_CO")),
...
export const apiKeyOption = Options.redacted("api-key").pipe(
  Options.optional,
  Options.withDescription("API key for indexing.co (overrides API_KEY_INDEXINGCO env var)")
)

// Helper function to get API key
export const getApiKey = (cliApiKey: Option.Option<Redacted.Redacted>) =>
  Effect.gen(function*() {
    const key = yield* (Option.isSome(cliApiKey)
      ...
      : Config.option(apiKeyConfig).pipe(
        ...
        Effect.catchAll(() =>
          Effect.gen(function*() {
            yield* Console.error(
              "API key is required. Set API_KEY_INDEXINGCO environment variable or use --api-key option"
            )
